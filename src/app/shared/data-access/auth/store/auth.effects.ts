...
export const updateMe = createEffect(
  (actions$ = inject(Actions), authService = inject(AuthService)) => {
    return actions$.pipe(
      ...
      exhaustMap(({ user, callback }) =>
        authService.updateMe(user).pipe(
          tap(() => callback()),
...
export const updatePassword = createEffect(
  (actions$ = inject(Actions), authService = inject(AuthService)) => {
    return actions$.pipe(
      ...
      exhaustMap(({ passwordUpdateData, callback }) =>
        authService.updatePassword(passwordUpdateData).pipe(
          tap(() => callback()),
          map(data => authActions.updatePasswordSuccess({ user: data?.data?.data })),
          catchError(error =>
            of(authActions.updatePasswordFailure({ error: error?.error?.message })),
          ),
        ),
      ),
    );
