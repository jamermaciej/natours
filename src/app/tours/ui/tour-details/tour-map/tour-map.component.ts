...
export class TourMapComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    if (this.locations.length > 1 ) {
      this.map.fitBounds(this.bound, { top: 170, bottom: 170 });
    }
  }

  setCenterPoint() {
    for(let location of this.locations) {
      this.bound.extend( new google.maps.LatLng(location.coordinates[1], location.coordinates[0]) );
    }
    
    this.center = {
      lat: this.bound.getCenter().lat(),
      lng: this.bound.getCenter().lng()
    }
  }
}
